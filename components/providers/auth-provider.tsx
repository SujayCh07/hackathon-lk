...
import mockAccount from '@/src/data/mockAccount.json'
    ...
    async (authUser: User) => {
      ...
      try {
        ...
        const fallbackBalance =
          typeof mockAccount?.balanceUSD === 'number' && !Number.isNaN(mockAccount.balanceUSD)
            ? mockAccount.balanceUSD
            : 0
        const fallbackTransactions = Array.isArray(mockTransactions) ? mockTransactions : []
        setNessie({
          customerId: (authUser.user_metadata?.nessieCustomerId as string | undefined) ?? null,
          accounts: [
            {
              id: 'demo-account',
              name: 'Demo Account',
          ...
          transactions: normaliseTransactions(fallbackTransactions as unknown[]),
        })
      } finally {
